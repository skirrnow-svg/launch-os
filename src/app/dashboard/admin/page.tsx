 ...
 * Admin-only, org-scoped page to view/update the integration API keys the
 * platform uses for AI orchestration and asset generation. Keys are write-only
 * from the UI (shown masked, never returned in full) and persisted encrypted
 * server-side via PATCH /api/admin/settings.
 *
 * TODO(phase-1): gate behind an admin role check (redirect non-admins) once
 * Clerk org roles are wired; load current masked values from the API on mount.
 */
...
export default function AdminSettingsPage() {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    setStatus("saving");
    setMessage("");
    // Only send fields the admin actually typed into (don't overwrite with blanks).
